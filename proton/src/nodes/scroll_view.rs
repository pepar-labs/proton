...
pub struct ScrollViewNode {
    ...
    /// Vertical scroll offset in pixels (how far the content is scrolled down)
    pub scroll_offset: f32,
}

impl Default for ScrollViewNode {
    fn default() -> Self {
        Self {
            children: Vec::new(),
...
impl ScrollView {
    ...
    /// Create a vertical scroll view (default)
    ...
    pub fn build(self) -> Node {
        Node::ScrollView(self.node)
    }
}

impl From<ScrollView> for Node {
    fn from(builder: ScrollView) -> Node {
        builder.build()
    }
}
